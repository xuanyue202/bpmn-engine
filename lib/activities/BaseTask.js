...
function sequentialConditionLoop(message, callback) {
  ...
  syncUntil = function (test, iterator, callback, err, a, b, c) {
      if (!test()) {
          iterator(function (err, a, b, c) {
              if (err) {
                  return callback(err);
              }
              syncUntil(test, iterator, callback, err, a, b, c);
          });
      }
      else {
          callback(err, a, b, c);
  ...
  executeFn((err, a, b, c) => {
      ...
      syncUntil(executeFn, testFn, callback, err, a, b, c);
  })
...
function sequentialCollectionLoop(message, callback) {
  ...
  forEachSeries = function (arr, iterator, callback) {
    ...
    forEachSeries(collection, executeFn, callback);
