...
function sequentialConditionLoop(message, callback) {
  ...
  console.warn(" async.doUntil is called, check whether this will raise bugs!!!");
  async.doUntil(executeFn, testFn, callback);
  // let result = null; 
  // do {
  //   let err = null;
  //   executeFn((errParam, resultParam) => {
  //     err = errParam;
  //     result = resultParam;
  //   });
  //   if(err) {
...
function sequentialCollectionLoop(message, callback) {
  ...
  /*for(let i = 0; i < collection.length; i++){
    let now = collection[i];
    let err = null;
    executeFn(now, (errParam) => {err = errParam;});
    if (err) {
      return callback(err);
    }
  }
