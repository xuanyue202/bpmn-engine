...
internals.Engine.resume = function(engineState, executeOptionsOrCallback, callback) {
  ...
  function resume() {
    ...
    for(let i = 0; i < engineState.definitions.length; i++){
      let definitionState = engineState.definitions[i];
      let err = null;
      const definition = mapper.Definition.resume(definitionState, options, (errParam) => {err = errParam;});
      if (err) {
        return resumeCallback.call(engine, err);
      }
      engine.definitions.push(definition);
      setupDefinition.call(engine, definition);
    }
...
internals.Engine.prototype.execute = function(executeOptionsOrCallback, callback) {
  ...
  this.getDefinitions((getErr, definitions) => {
    ...
    for(let i = 0; i < definitions.length; i++){
      let d = definitions[i];
      let err = null;
      d.execute(executeOptions, (errParam) => {err = errParam;});
      if (err) {
        return executeCallback.call(this, err, definitions[0], definitions);
      }
    }
    executeCallback.call(this, null, definitions[0], definitions);
  });
};
...
function loadDefinitions(callback) {
  if (this.sources.length) {
    ...
    let result = this.sources.map(loadDefinitionBySource.bind(this));
