...
internals.Engine.prototype.getInstance = function(optionsOrCallback, callback) {
  ...
  if (typeof optionsOrCallback === 'function') {
   ...
   /*Async.waterfall([
    ...
    getContext.call(this, options)((err, definition, moddleContext)=>{
      if(err) return callback(err);
      this.id = getPropertyValue(moddleContext, 'rootHandler.element.id', 'anonymous');
      this.definition = definition;
      this.moddleContext = moddleContext;
      validation.validate.bind(null)(moddleContext, (err) => {
        if(err) return callback(err);
        loadProcesses.call(this, this.moddleContext, options);
        if (!this.entryPointId) {
          return callback(new Error(`definition <${this.id}> has no executable process`));
        }
        return callback(null, this.initProcess, this.processes);
      });
    });
};
...
function teardownProcesses() {
  this.processes.forEach((p) => {
    if(this._onMessage) p.removeListener('message', this._onMessage);
    if(this._onEnd) p.removeListener('leave', this._onEnd);
  });
}

function sourceHash(buffer) {
  return "1";//crypto.createHash('md5').update(buffer).digest('hex');
}
